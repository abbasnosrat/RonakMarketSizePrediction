...
def train_booster(df_t, params, train_size=-5):
    ...
    dg = df_t.copy().rename(columns={"QtyAdadi":"y"})
...
def XGB(manual, df_t, train_size, horizon):
    ...
    fig_tuned.update_layout(title='One Step Ahead Prediction', xaxis_title='Date', yaxis_title='QtyAdadi')
    st.markdown("""
        <style>
        .chart-container {
            position: relative;
...
def FBProphet(manual, df_t,test_size_manual, horizon):
    ...
    dg = dg[["ds", "QtyAdadi"]].rename(columns={"QtyAdadi":"y"})
    # dg
